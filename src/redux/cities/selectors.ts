...
const createGetFindDefaultCityByName = (name: string) => (state: StoreState) =>
    find(getDefaultCities(state), ({ name: _name }) => name === _name);

const getSavedCities = (state: StoreState) => getState(state).savedCities;

const createGetFindSavedCityById = (id: number) => (state: StoreState) =>
    find(getSavedCities(state), ({ id: _id }) => id === _id);
...
const CitiesSelectors = {
    getDefaultCities,
    getSavedCities,
    createGetFindSavedCityById,
    createGetFindDefaultCityByName,
    getError
};
